...
public class Evaluator {
    ...
    private Evaluator(FieldParameters field, MoveParameters move, BotSettings settings) {
        this.field = field;
        this.move = move;
    ...
    public static Evaluator GetDefault(BotSettings settings) {
        if (settings.Genes is null) {
    ...
    public static Evaluator CreateFromGeneValues(IEnumerable<int> geneCollection) {
        ...
        for (int i = 0; i < targets.Length; i++) {
            ...
            foreach (FieldInfo field in target.GetType().GetFields()) {
                if (field.FieldType == typeof(int)) {
                    ...
                    Console.Error.WriteLine($"Evaluator.CreateFromGeneValues: type {field.FieldType} is not supported");
        ...
        return new Evaluator(fieldParams, moveParams, new BotSettings());
    }
